...
def analysis(L1):
    ...
    for x in last7x7Days:
        Counlast7Days += x
    print(
        f"last seven days you studied \033[38;2;255;0;255m{round(current7days,2)}\033[0m hrs which is ", end="")
    if current7days > Counlast7Days:
        print(
            f"\033[38;2;0;255;0m{round(current7days - Counlast7Days,2)}\033[0m hrs more than last week !")
    else:
        print(
            f"\033[38;2;255;0;0m{round(Counlast7Days - current7days,2)}\033[0m hrs less than last week \033[38;2;255;0;255m:(\033[0m")
...
def cmdGiven():
    ...
    def decider():
        ...
        if cmd == "0":
            ...
            if not breakList:
                if len(studyList) == 0:
                    print("ara ara hopefully you last longer this time fufu~\n")
                temp = strftime("%H %M", current_time)
                temp += "\n"
                ...
                if len(studyList) == 2:
                    ...
                    if len(studyHoursList) == 2:
                        temp = str(
                            float(studyHoursList[0][:-1]) + float(studyHoursList[1][:-1]))
                        temp += "\n"
                        studyHoursList.clear()
                        studyHoursList.append(temp)
                        cmdGiven()
            ...
            if not studyList:  # true if studyList is empty
                ...
                if len(breakList) == 2:
                    ...
                    if len(breakHoursList) == 2:
                        temp = str(
                            float(breakHoursList[0][:-1]) + float(breakHoursList[1][:-1]))
                        temp += "\n"
                        breakHoursList.clear()
                        breakHoursList.append(temp)
                        cmdGiven()
            ...
            if yesMaster:
                ...
                if yesMaster:
                    while True:
                        ...
                        yesMaster = input(
                            "specify the number on the left of title to download :")

                    print(
                        f"downloading doc associated with {titles[yesMaster]}")
                    ...
                    ara.urlretrieve(
                        url, f"/storage/emulated/0/Download/{nameForFile}pdf")
                    ara.urlcleanup()
                    print("done")
      ...
      cmdGiven()"""
            ...
            if not yesMaster:
                ...
                if algo == "P":
                    ...
                    for a, b, c in zip(P_topicNameList, P_timesRevisedList, P_daysAgoList):
                        if i % 2 == 0:
                            print(
                                f"{i}   \033[38;2;0;170;170m{c} days ago\033[0m   {a}")
                        else:
                            print(
                                f"{i} - \033[38;2;0;170;170m{c} days ago\033[0m - {a}")
                        i += 1
                    try:
                        ...
                        P_timesRevisedList[whichOne] = str(
                            int(P_timesRevisedList[whichOne]) + 1)
                        print(
                            f"ok so you are going to revise \033[38;2;0;170;170m{P_topicNameList[whichOne]}\033[0m")
                        temp = []
                        for a, b, c in zip(P_topicNameList, P_timesRevisedList, P_daysAgoList):
                            temp.append(f"{a},{b},{c}\n")
                        with open("Prevision.txt", mode="w") as savingfile:
                ...
                # elif algo == "P":
                    ...
                    try:
                        ...
                        C_timesRevisedList[whichOne] = str(
                            int(C_timesRevisedList[whichOne]) + 1)
                        print(
                            f"ok so you are going to revise \033[38;2;0;170;170m{C_topicNameList[whichOne]}\033[0m")
                        temp = []
                        for a, b, c in zip(C_topicNameList, C_timesRevisedList, C_daysAgoList):
                            temp.append(f"{a},{b},{c}\n")
                        with open("Crevision.txt", mode="w") as savingfile:
                ...
                # elif algo == "C":
                    ...
                    try:
                        ...
                        M_timesRevisedList[whichOne] = str(
                            int(M_timesRevisedList[whichOne]) + 1)
                        print(
                            f"ok so you are going to revise \033[38;2;0;170;170m{M_topicNameList[whichOne]}\033[0m")
                        temp = []
                        for a, b, c in zip(M_topicNameList, M_timesRevisedList, M_daysAgoList):
                            temp.append(f"{a},{b},{c}\n")
                        with open("Mrevision.txt", mode="w") as savingfile:
            ...
            for a in pdfList:
                ...
                refinedName.replace("  ", " ")
                rename(f"{saveFrom}{pdfList[i]}",
                       f"{pathToSaveAt}/{refinedName}")
                i += 1
            print("done")
            cmdGiven()
            ...
            mamamulti = 1 + PlsMulti(returnerOfWorlds=True) + \
                aCurrentStreak*(0.0166666667) + (hrsclaims - 8)*0.0625
            print(
                f"total coin gain multi = {round((mamamulti - 1)*100,2)} % bonus coins or \033[38;2;170;85;0m{mamamulti}\033[0m × 3 = \033[38;2;255;0;255m{mamamulti*3}\033[0m coins")
            cmdGiven()
            ...
            while True:
                try:
                    wryy = input(
                        "did u eat? ehehhehe (yes: x Kcals, no blank enter :")
                    if wryy:
                        i = i + int(wryy)
                    else:
                        break
...
noChangeI = 0
while noChangeI < 6:
    OneoutOfsix = ("7dayBreak", "7dayStudy", "breakHours",
                   "breakList", "studyHours", "studyList")
    HaveTosaveList = (SevendayBreakList, SevendayStudyList,
                      breakHoursList, breakList, studyHoursList, studyList)
